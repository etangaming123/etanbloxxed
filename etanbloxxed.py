...
rpcenabled = True # whether the rpc is enabled or not
...
def find_latest_log_file(directory): # gpt written
    ...
    actualfiles = [f for f in files if "Player" in os.path.basename(f)]
    latest_file = max(actualfiles, key=os.path.getmtime)
    return latest_file
...
def getgamedetails(universeid): # getting the details of the game, like name, publisher etc
    ...
    while True:
        try:
            ...
            url = f'https://games.roproxy.com/v1/games?universeIds={universeid}'
            ...
            if notrealdatalol:
                ...
                name = data['name']
                if 'hasVerifiedBadge' in data['creator'].keys():
...
def getUserPFP(): # user pfp as url so that it shows up on discord
    ...
    while True:
        ...
        try:
            url = f'https://thumbnails.roproxy.com/v1/users/avatar-headshot?userIds={userid}&size=48x48&format=Png&isCircular=false'
            response = requests.get(url)
...
def getUsername(): # username and displayname
    ...
    while True:
        ...
        try:
            url = f'https://users.roproxy.com/v1/users/{userid}'
            response = requests.get(url)
            if response.status_code == 429:
                time.sleep(1)
            elif response.status_code == 404:
...
def getImageAssetId(placeid): # game image
    ...
    while True:
        ...
        try:
            url = f'https://thumbnails.roproxy.com/v1/places/gameicons?placeIds={placeid}&size=150x150&format=Png'
            response = requests.get(url)
            if response.status_code == 429:
                time.sleep(1)
            elif response.status_code == 404:
                return 'etanbloxxed_error'
            elif response.status_code == 200:
...
def getImageUrl(asdf): # bloxstrap rpc custom images ig
    imageid = str(asdf).replace("rbxassetid://", "")
    global cache
    if not imageid in cache.keys():
        attemptno = 1
        while True:
            printTemporary(f'Getting image url... Attempt {attemptno}')
            try:
                url = f'https://thumbnails.roproxy.com/v1/assets?assetIds={imageid}&size=150x150&format=Png'
                response = requests.get(url)
                if response.status_code == 200:
                    data = response.json()["data"][0]
                    if not data['imageUrl'] == None or data['imageUrl'] == "":
                        cache[imageid] = data['imageUrl']
                        return data['imageUrl']
                else:
                    return ""
                attemptno += 1
...
# --rpc--
def updateRpc(newrpc, placeid): # yeah
    global hasRPCwithextras, imageassetid, bloxstrapRPCCustomState
    if rpcenabled:
        try:
            if imageassetid == '':
        ...
        if command_data['command'] == 'SetRichPresence':
            ...
            if 'largeImage' in data:
                if 'assetId' in data['largeImage']:
                    imageurl = getImageUrl(data['largeImage']['assetId'])
                    if imageurl != '':
                        current_state['large_image'] = imageurl
            ...
            if 'smallImage' in data:
                if 'assetId' in data['smallImage']:
                    imageurl = getImageUrl(data["smallImage"]["assetId"])
                    if imageurl != "":
            ...
            current_state['buttons'] = [{'label': 'get etanbloxxed', 'url': 'https://github.com/etangaming123/etanbloxxed'}, {'label': 'My Current Game', 'url': f'https://www.roproxy.com/games/{placeid}/'}]
            RPC.update(**current_state)
            print(f'state = {current_state['state']}, details = {current_state['details']}')

def idleRpc(): # set rpc to idle
    global hasRPCwithextras, rpcenabled
    if rpcenabled:
        hasRPCwithextras = False
        try:
...
def theactualetanbloxxedshi(logfile, readexisting):
    global placeid, universeid, creatorname, gamename, imageassetid, cachedstatus, cachedipadress, bloxstrapRPCCustomState, close, rpcenabled
...
if __name__ == '__main__': # idk why but gpt added this so (im kidding it has somethign to do with modules or something IDK)
    ...
    while True: # HERE COMES ALL THE NESTED CODE
        ...
        if userinput == 'cmds':
            ...
            cmds = {'open': 'runs roblox', 'settings': 'modify your settings', 'terminate': 'terminates (force quits) roblox', 'exit': 'exits etanbloxxed'}
            ...
            with open(log_file, 'r', encoding='utf-8') as logfile:
                theactualetanbloxxedshi(logfile, False)
        
        elif userinput == 'scan':
            print("Scanner is active, etanbloxxed will automatically run when Roblox is opened.")
            lastlog = find_latest_log_file(log_path)
            while True:
                try:
                    currentlog = find_latest_log_file(log_path)
                    if currentlog != lastlog:
                        print('New log file detected, running etanbloxxed...')
                        with open(currentlog, 'r', encoding='utf-8') as logfile:
